...
class DataTrainingArguments:
    ...
    output_name: Optional[str] = field(default='results', metadata = {"help": "PR custom: specify name for test results .csv"}
    )
...
def main():
    ...
    logger.info(f"Testing parameters {training_args}")
    ...
    pred_results = trainer.predict(test_dataset=test_dataset) 

    output_test_file = os.path.join(training_args.output_dir, f"{data_args.output_name}.csv")
    ...
    labels.merge(predictions).to_csv(output_test_file, index = False)
